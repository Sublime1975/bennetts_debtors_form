...
interface BankNameFieldProps {
  label: string;
  value: string;
  onChange: (value: string) => void;
  /** Called with the bank's universal branch code when a listed bank is picked. */
  onSelectBank?: (universalBranchCode: string) => void;
  error?: string;
  required?: boolean;
  id?: string;
  placeholder?: string;
...
export function BankNameField({ label, value, onChange, onSelectBank, error, required, id, placeholder }: BankNameFieldProps) {
  ...
  const matches = query.length === 0 ? SA_BANKS : SA_BANKS.filter((b) => b.name.toLowerCase().includes(query));
          ...
          {matches.map((bank) => (
            <li key={bank.name}>
              <button
                ...
                onClick={() => {
                  onChange(bank.name);
                  onSelectBank?.(bank.universalBranchCode);
                  setOpen(false);
                }}
              ...
              >
                <span>{bank.name}</span>
                <span className="font-body text-xs text-muted">{bank.universalBranchCode}</span>
              </button>
            </li>
          ))}
        </ul>
